...
sollten Sie daher versuchen,
das **Originaldokument** (z. B. eine ``.docx``-Datei in Word/Writer oder eine ``.odt``-Datei in LibreOffice) zu ändern und anschließend neu als PDF zu exportieren.
Wenn Ihnen das Original nicht vorliegt,
bieten die folgenden Werkzeuge unter Linux Mint praktische Auswege.
...
* **PDF Arranger:**
  Dieses schlanke Open-Source-Werkzeug empfehle ich Ihnen wärmstens für diese Aufgaben.
  Sie können PDFs einfach per Drag-and-Drop in das Fenster ziehen,
...
Möchten Sie Tippfehler direkt im PDF korrigieren oder Grafiken austauschen,
stehen Ihnen dafür verschiedene Ansätze zur Verfügung:
...
* **LibreOffice Draw (kostenlos & Open Source):**
  ...
  dass Draw zusammenhängende Absätze oft in einzelne Zeilen zerlegt,
  was das Formatieren von Fließtext erschwert.
...
* **Master PDF Editor (proprietär, eingeschränkt kostenlos):**
  ...
  .. note::
     Achten Sie darauf, dass manche erweiterte Funktionen in der kostenlosen Version ein Wasserzeichen auf dem gespeicherten Dokument hinterlassen.
     Für reine Textänderungen in bestehenden Textfeldern ist es dennoch eines der besten Werkzeuge unter Linux.
...
* **Adobe Online-Konverter (Web-basiert):**
  ...
  In solchen Fällen empfehle ich Ihnen den kostenlosen `Online-Konverter von Adobe <https://www.adobe.com/de/acrobat/online/pdf-to-word.html>`_,
...
Nicht jedes Formular ist digital interaktiv.
Oft müssen Sie ein klassisches PDF-Dokument am PC ausfüllen oder digital unterschreiben.

* **Xournal++:**
  Das perfekte Werkzeug für handschriftliche Notizen, Skizzen und das Einfügen einer **digitalen Unterschrift**.
  Sie können mit der Maus oder einem Grafiktablett direkt auf dem Dokument unterschreiben.
  Zudem können Sie über die Textwerkzeuge ein Textfeld flexibel über jede beliebige Stelle des Dokuments legen,
...
* **Formulare erstellen:**
  ...
  Aktivieren Sie diese unter ``Ansicht -> Symbolleisten -> Formular-Steuerelemente``.
...
* **Formulare ausfüllen:**
  ...
  Als stabilen und zuverlässigen PDF-Formular-Ausfüller empfehle ich Ihnen den **Chromium-Webbrowser** (oder Google Chrome).
...
Linux Mint bringt zwar ein Standard-Scanprogramm mit,
für produktives Arbeiten empfehle ich Ihnen jedoch **NAPS2** (Not Another PDF Scanner 2).
...
als auch automatische Dokumenteneinzüge (ADF).
Sie können gescannte Seiten schnell verschieben, drehen, zuschneiden und direkt als PDF speichern.
...
das offizielle Debian-Paket (``.deb``) von der Entwicklerseite zu installieren.
...
Das erzeugte PDF-Dokument sieht optisch genauso aus wie der Original-Scan,
aber Sie können nun Text mit der Maus markieren,
